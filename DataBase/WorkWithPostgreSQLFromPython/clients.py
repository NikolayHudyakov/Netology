...
class Clients:
    ...
    def find_client(self, first_name: str='%', last_name: str=None, email: str=None, phone: str=None):
        with self.conn.cursor() as cur:
            if first_name != None:
...
# А вот закрытие соединения он не делает, поэтому close() вызывать все же стоит;
# функционал поиска клиентов все же лучше было реалзиовать не только по конкретным параметрам, но и по любой их комбинации. 
# Можно было так: в качестве значений по умолчанию у параметров делаете не None, а пустые шаблоны ('%'). 
# Когда эти параметры не будут заданы, то в рамкам select-запроса в результат будут попадать строки, которые соответствуют указанным значениям,
#  а во всех остальных у нас может быть хоть что. Это как раз нам и надо :)
# рекомендовал бы вам почитать вот этот материал для понимания того, как можно динамически формировать запросы на основе сторонних переменных;
# python - язык с динамической типизацией и строго задать тип параметра функции нельзя. Можно лишь прописать необходимые проверки на тип объекта вручную.
